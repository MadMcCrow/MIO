...
inline GLSkeleton& GLSkeleton::operator= (Skeleton MIOSkeleton){
   ...
   for (size_t bone = 0; bone < it; bone++ )
   {
       ...
       ///Getting the WORLD translation
       {
           ...
           MIOSkeleton.getSkeleton().at(bone).getBoneGlobalTranslation()[0],
           MIOSkeleton.getSkeleton().at(bone).getBoneGlobalTranslation()[1],
           MIOSkeleton.getSkeleton().at(bone).getBoneGlobalTranslation()[2]);
           WorldTranslate = trs;
       }
       ///Getting the proper scale/rotation/translate to parent.
       {
           QVector3D ltrs = QVector3D(
           MIOSkeleton.getSkeleton().at(bone).getBoneLocalTranslation()[0],
           MIOSkeleton.getSkeleton().at(bone).getBoneLocalTranslation()[1],
           MIOSkeleton.getSkeleton().at(bone).getBoneLocalTranslation()[2]);
...
inline GLSkeleton GLSkeleton::import(Skeleton MIOSkeleton)
{
    GLSkeleton retval;
    retval = MIOSkeleton;
    return retval;
